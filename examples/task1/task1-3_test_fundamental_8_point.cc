...
FundamentalMatrix fundamental_8_point (math::Matrix<double, 3, 8> const& points1
    ...
    for(int i = 0; i < 8; ++i) {
        math::Vector<double,3> p1 = points1.col(i);
        math::Vector<double,3> p2 = points2.col(i);
        double u1 = p1[0], v1 = p1[1];
        double u2 = p2[0], v2 = p2[1];
        A(i, 0) = u2 * u1;  A(i, 1) = u2 * v1;  A(i, 2) = u2 * 1.0;
        A(i, 3) = v2 * u1;  A(i, 4) = v2 * v1;  A(i, 5) = v2 * 1.0;
        A(i, 6) = 1.0 * u1; A(i, 7) = 1.0 * v1; A(i, 8) = 1.0 * 1.0;
    }

    
    math::Matrix<double, 8, 9> UA;
    math::Matrix<double, 9, 9> SA, VA;
    math::matrix_svd<double, 8, 9> (A, &UA, &SA, &VA);
    math::Vector<double, 9> f = VA.col(8);
    f.normalize();
    for(int i = 0; i < 3; ++i) {
        for(int j = 0; j < 3; ++j) {
            F(i, j) = f[i*3+j];            
    ...
    math::Matrix<double, 3, 3> UF, SF, VF;
    math::matrix_svd<double, 3, 3> (F, &UF, &SF, &VF);
    SF(2,2) = 0.0;
