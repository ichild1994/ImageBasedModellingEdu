              ...
              double* point_x_ptr, double* point_y_ptr)
{
    ...
    double u, v;
    double RX, RY, RZ;
    RX = R[0] * X[0] + R[1] * X[1] + R[2] * X[2];
    RY = R[3] * X[0] + R[4] * X[1] + R[5] * X[2];
    RZ = R[6] * X[0] + R[7] * X[1] + R[8] * X[2];
    xc = RX + t[0];
    yc = RY + t[1];
    zc = RZ + t[2];
    x = xc / zc;
    ...
    double Ju_x, Ju_y;
    double Jv_x, Jv_y;
    Ju_x = f*x * (k0+k1*2.0*r2)*2.0*x + f*d;
    Ju_y = f*x * (k0+k1*2.0*r2)*2.0*y;  
    Jv_x = f*y * (k0+k1*2.0*r2)*2.0*x;  
    Jv_y = f*y * (k0+k1*2.0*r2)*2.0*y + f*d;

    double Jx_xc, Jx_yc, Jx_zc;
    double Jy_xc, Jy_yc, Jy_zc;
    Jx_xc = 1.0/zc; Jx_yc = 0; Jx_zc = -x/zc;
    Jy_xc = 0; Jy_yc = 1.0/zc; Jy_zc = -y/zc;

    double Jxc_w0, Jxc_w1, Jxc_w2;
    double Jyc_w0, Jyc_w1, Jyc_w2;
    double Jzc_w0, Jzc_w1, Jzc_w2;
    Jxc_w0 = 0; Jxc_w1 = RZ; Jxc_w2 = -RY;
    Jyc_w0 = -RZ; Jyc_w1 = 0; Jyc_w2 = RX;
    Jzc_w0 = RY; Jzc_w1 = -RX; Jzc_w2 = 0;

    double Ju_xc, Ju_yc, Ju_zc;
    double Jv_xc, Jv_yc, Jv_zc;
    Ju_xc = Ju_x * Jx_xc + Ju_y * Jy_xc;
    Ju_yc = Ju_x * Jx_yc + Ju_y * Jy_yc;
    Ju_zc = Ju_x * Jx_zc + Ju_y * Jy_zc;
    Jv_xc = Jv_x * Jx_xc + Jv_y * Jy_xc;
    Jv_yc = Jv_x * Jx_yc + Jv_y * Jy_yc;
    Jv_zc = Jv_x * Jx_zc + Jv_y * Jy_zc;
    ...
    cam_x_ptr[6] = Ju_xc*Jxc_w0 + Ju_yc*Jyc_w0 + Ju_zc*Jzc_w0;
    cam_x_ptr[7] = Ju_xc*Jxc_w1 + Ju_yc*Jyc_w1 + Ju_zc*Jzc_w1;
    cam_x_ptr[8] = Ju_xc*Jxc_w2 + Ju_yc*Jyc_w2 + Ju_zc*Jzc_w2;
    cam_y_ptr[6] = Jv_xc*Jxc_w0 + Jv_yc*Jyc_w0 + Jv_zc*Jzc_w0;
    cam_y_ptr[7] = Jv_xc*Jxc_w1 + Jv_yc*Jyc_w1 + Jv_zc*Jzc_w1;
    cam_y_ptr[8] = Jv_xc*Jxc_w2 + Jv_yc*Jyc_w2 + Jv_zc*Jzc_w2;

    // derivation on pos
    point_x_ptr[0] = Ju_xc*R[0] + Ju_yc*R[3] + Ju_zc*R[6];
    point_x_ptr[1] = Ju_xc*R[1] + Ju_yc*R[4] + Ju_zc*R[7];
    point_x_ptr[2] = Ju_xc*R[2] + Ju_yc*R[5] + Ju_zc*R[8];
    point_y_ptr[0] = Jv_xc*R[0] + Jv_yc*R[3] + Jv_zc*R[6];
    point_y_ptr[1] = Jv_xc*R[1] + Jv_yc*R[4] + Jv_zc*R[7];
    point_y_ptr[2] = Jv_xc*R[2] + Jv_yc*R[5] + Jv_zc*R[8];
