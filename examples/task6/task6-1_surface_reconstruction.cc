...
fssrecon (AppOptions const& app_opts, fssr::SampleIO::Options const& pset_opts)
{
    ...
    for(int i = 0; i < voxels.size(); ++i) {
        auto &vdata = voxels[i].second;
        
        //printf("voxel(%d): %f", vdata.value);
        vdata.value += -50.0f;
    }
    ...
    for(int i = -50; i <= 50; i+=10) {
        ...
        */
        ...
        std::string outfile = std::string(app_opts.out_mesh.begin(), app_opts.out_mesh.end()-4);
        outfile += std::to_string(i)+".ply";
        core::geom::save_ply_mesh(mesh, outfile, ply_opts);
